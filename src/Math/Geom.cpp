...
RayTest::RayTest(Plan _p)
{
	p = _p;
}

bool RayTest::test(glm::vec3 &v, int x, int y, glm::vec3 campos)
{
    GLdouble Bx,By,Bz;
    GLdouble Mx,My, Mz,t;
    GLint viewport[4];
    ...
    gluUnProject( x,viewport[3]-y,0,mvmatrix,projmatrix,viewport,&Bx,&By,&Bz);
	glm::vec3 B(Bx,By,Bz);
	glm::vec3 AB= B - campos;
	Droite d_AB = Droite(campos,AB); 
	return p.intersect(d_AB,v);
}

Plan::Plan(glm::vec3 p, glm::vec3 n)
{
	this->normal = n;
...
Droite::Droite(glm::vec3 ori, glm::vec3 dir)
{
	this->dir = dir;
...
Droite D (P1 = Point(x,y,z), V = Vecteur(u,v,w))
Plan p (P2 = Point(_x,_y,_z) N = Normal(_u,_v,_w))

Intersect seulement si V.N != 0 
...
==>
Intesect : _u*(x + k*u)+ _v*(y + k*v) + _w*(z + k*w) + d = 0

==>
k*u*_u + k*v*_v + k*w*_w + _u*x + _v*y + _w * z + d = 0
...
*/
bool Plan::intersect(Droite d, glm::vec3 &v)
{
	
	if(ABS(glm::dot(d.dir,normal)) < 0.0001f)
		return false;

	float _d = glm::dot(normal,p);
	float _C = glm::dot(normal,d.dir);
	float C = glm::dot(normal,d.ori) + _d;
